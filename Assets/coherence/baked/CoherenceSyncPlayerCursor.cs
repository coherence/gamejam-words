...
namespace Coherence.Generated
{
	...
	public class CoherenceSyncPlayerCursor : CoherenceSyncBaked
	{
		...
		private InputBuffer<PlayerInput> inputBuffer;
		...
		protected void Awake()
		{
			...
			inputBuffer = new InputBuffer<PlayerInput>(coherenceInput.InitialBufferSize, coherenceInput.InitialBufferDelay, inputsRequireSubsequentFrames);
		...
		private void Update()
		{
			...
			{
				PerformInterpolation(Time.deltaTime);
			}
		}
		...
		private void FixedUpdate()
		{
			...
			{
				PerformInterpolation(Time.fixedDeltaTime);
			}
		}
		...
		static byte[] ListBytes4096ToByteArray(in FixedListByte4096 list)
		{
			return CoherenceToUnityConverters.FromUnityFixedListByte4096(list);
		}

		static FixedListByte4096 ObjectToListBytes4096(object o)
		{
			return CoherenceToUnityConverters.ToUnityFixedListByte4096((byte[])o);
		}
		...
		private void PerformInterpolation(float deltaTime)
		{
			if (coherenceSync.isSimulated)
			{
				return;
			...
			var localFrame = (ulong)client.ClientSimulationFrame.Frame;
			...
			foreach (var change in orderedChanges)
			{
				...
				switch(componentType)
				{
					...
					case 71:
					{
						...
						if((mask & 0b00000000000000000000000000000001) != 0)
						{
							_player.playerName = (String)(data.playerName).ToString();
						}
		...
		private void SendInputState()
		{
			...
			while (inputBuffer.DequeueForSending(currentSimulationFrame, out long frameToSend, out PlayerInput input, out bool differs))
			{
			...
			case "key":
				{
					...
					inputBuffer.TryGetInput(frame, out PlayerInput input);
		...
		{
			var input = (PlayerInput)entityInput;
